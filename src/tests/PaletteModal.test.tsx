...
import PaletteModal from '@/components/NeoPixels/PaletteModal/PaletteModal';
...
describe('Palette Modal component', () => {
  ...
  it('should handle submit button click', async () => {
    const { getByTestId } = render(<PaletteModal device={device} opened close={() => {}} />);
  ...
  it('should call the given close function when close button is clicked', async () => {
    ...
    const { getByTestId } = render(<PaletteModal device={device} opened close={close} />);
